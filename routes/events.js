...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (Calendar) {
        const events = await eventDAO.getAll(req.params.calenderId);
...
router.delete("/:id", async (req, res, next) => {
  try {
      ...
      if (!deleteEvent || !calendar || event.calendarId != req.params.calendarId) {
          res.sendStatus(404).send ('event does not exist');
      } else {
        res.sendStatus(200);
...
//         const event = await eventDAO.getById(req.params.id);
//         const deleteEvent = await eventDAO.removeById( req.params.id) 
//         if (event && event.calendarId.toSting() === calendarId) {
//             await eventDAO.removeById(event)
//             res.sendStatus(200);
//         } else {
